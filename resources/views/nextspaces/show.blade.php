...
<x-app-layout>
    <div class="bg-gray-50 min-h-screen">
        ...
        <div class="relative w-full h-48 bg-cover bg-center" style="background-image: url('{{ $nextspace->image ?? 'https://placehold.co/1200x300/F8F9FA/6B7280?text=NextSpace' }}');">
            <div class="absolute inset-0 bg-black/30"></div>
            
            {{-- Back button --}}
            <div class="absolute top-3 left-3 z-20">
                <a href="{{ route('dashboard') }}" class="inline-flex items-center px-3 py-1.5 bg-white/90 backdrop-blur-sm text-gray-700 rounded-lg hover:bg-white transition-all text-sm shadow-sm">
            ...
            <div class="bg-white shadow-sm rounded-xl overflow-hidden border border-gray-100">
                ...
                <div class="p-5 border-b border-gray-100">
                    <div class="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4">
                        <div class="flex-1">
                            <h1 class="text-xl font-bold text-gray-900 mb-2">{{ $nextspace->title }}</h1>
                            ...
                            <div class="flex flex-wrap items-center gap-3 mb-2">
                                @if($nextspace->rating)
                                    <div class="flex items-center gap-1">
                                        ...
                                        <span class="text-gray-700 font-medium text-sm">{{ number_format($nextspace->rating, 1) }}</span>
                                        @if($nextspace->reviews_count)
                                            <span class="text-gray-500 text-xs">({{ $nextspace->reviews_count }})</span>
                                        @endif
                                    </div>
                                @endif
                                ...
                                <form method="POST" action="{{ route('favorites.toggle', $nextspace->id) }}">
                                    @csrf
                                    <button type="submit" class="text-gray-400 hover:text-blue-500 transition-colors">
                                        @if($isFavorited)
                        ...
                        @if($nextspace->timeSlots && $nextspace->timeSlots->count())
                            <div class="lg:w-96">
                                <div class="bg-blue-50 rounded-lg p-4">
                                    <h3 class="text-sm font-medium text-blue-900 mb-3">Jam Tersedia</h3>
                                    <div class="grid grid-cols-4 gap-2">
                                        @foreach ($nextspace->timeSlots as $slot)
                                            <div class="text-center p-2 rounded-lg border text-xs font-medium transition-all
                                                {{ $slot->pivot->capacity <= 0 
                                                ...
                                                <div>{{ $slot->slot }}</div>
                                                <div class="text-xs mt-1 {{ $slot->pivot->capacity <= 0 ? 'text-gray-400' : 'text-blue-600' }}">
                ...
                @if($nextspace->description)
                    <div class="px-5 py-4 border-b border-gray-100">
                        ...
                        <p class="text-gray-600 text-sm leading-relaxed">{{ $nextspace->description }}</p>
                    </div>
                @endif
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-4 gap-5">
                        ...
                        <div class="lg:col-span-1">
                            <div class="bg-gray-50 rounded-lg p-4">
                                <h3 class="font-semibold text-gray-900 mb-3 text-sm">Kontak & Jam</h3>
                                
                                @if($nextspace->phone)
                                    <div class="mb-3">
                                        <p class="text-xs text-gray-500 mb-1">Telepon</p>
                                        <a href="tel:{{ $nextspace->phone }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium">{{ $nextspace->phone }}</a>
                                    </div>
                                @endif
                                        ...
                                        <div class="text-xs text-gray-700 space-y-1">
                                            @foreach($nextspace->hours as $hour)
                                                <div class="flex justify-between">
                                                    <span>{{ $hour->day_type === 'mon-fri' ? 'Sen-Jum' : 'Sab-Min' }}</span>
                                                    <span class="font-medium">{{ $hour->open_time }}-{{ $hour->close_time }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    @else
                        ...
                        <div class="lg:col-span-3 space-y-4">
                            ...
                            @if($nextspace->amenities && $nextspace->amenities->count())
                                <div>
                                    <h3 class="font-semibold text-gray-900 mb-3 text-sm">Fasilitas</h3>
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                                        @foreach ($nextspace->amenities as $amenity)
                                            <div class="flex items-center gap-2 p-2 bg-blue-50 rounded-lg text-xs">
                                                ...
                                                <span class="text-blue-800">{{ $amenity->name }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                            ...
                            @if($nextspace->services && $nextspace->services->count())
                                <div>
                                    <h3 class="font-semibold text-gray-900 mb-3 text-sm">Layanan & Harga</h3>
                    ...
                    <div class="px-5 py-4 border-t border-gray-100">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm text-gray-700 font-medium">{{ number_format($avgRating, 1) }}/5</span>
                                <span class="text-xs text-gray-500">({{ $reviewCount }} ulasan)</span>
                            </div>
                        </div>
                        
                        <div class="space-y-3 max-h-48 overflow-y-scroll border border-gray-200 rounded-lg p-3 bg-gray-50">
                            @foreach($nextspace->reviews as $review)
                                <div class="border-b border-gray-100 pb-3 last:border-b-0 bg-white rounded p-2">
                                    <div class="flex items-center gap-2 mb-1">
                                        <span class="font-medium text-gray-800 text-sm">{{ $review->user->name ?? 'Anonim' }}</span>
                ...
                <div class="bg-blue-50 p-5">
                    <div class="flex flex-col sm:flex-row gap-4 justify-between items-center">
                        <div class="text-center sm:text-left">
                            <h3 class="text-base font-semibold text-blue-900 mb-1">Siap untuk memesan?</h3>
                            <p class="text-blue-700 text-sm">Pesan tempat Anda dan nikmati ruang ini</p>
                        </div>

                        <div class="flex flex-col sm:flex-row gap-3 items-center">
                            @if($nextspace->base_price)
                                <div class="text-center">
                                    <div class="text-lg font-bold text-blue-900">Rp{{ number_format($nextspace->base_price, 0, ',', '.') }}</div>
                                    <div class="text-xs text-blue-600">harga mulai</div>
                                </div>
                            @endif
                            
                            <a href="{{ route('payment.form', ['nextspace_id' => $nextspace->id]) }}" class="inline-flex items-center justify-center bg-blue-600 text-white font-medium py-2.5 px-5 rounded-lg hover:bg-blue-700 transition-colors text-sm shadow-sm">
